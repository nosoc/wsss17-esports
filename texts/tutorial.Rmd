...
library(stringr)
library(proxy)
chat <- read_feather("~/tut_chat_df.rda")
...
View(filter(chat,clusterCut==1)) # Reflection of stream like retelling of seen and emotional responce
View(filter(chat,clusterCut==2)) # Reflection of stream like retelling of seen and emotional responce
View(filter(chat,clusterCut==3)) # Attempts to discuss actions and events (to some extent)
View(filter(chat,clusterCut==4)) # spammers against modes
...
View(filter(chat,clusterCut==8)) # bot
View(filter(chat,clusterCut==9)) # bot
...
We can assume that the most of the chat activity is tightly connected with the game events. Let's try to analyze association of chat communication and game events in one game. We already know how to analyze the association between the cluster and thematic category (Remember chanks 130-149). Now, please choose one game event and assess the relationship between game event and topics of communication. What is the interpretation of your findings?

```{r} 
match.df <- read_feather("~/tut_chat_df_task.rda")
``` 
...
You have a number of game events. They are binar, which means that message can be related to game event or not. What do some of them mean?
