...
```{r}
View(filter(chat,clusterCut==1)) # reactions on stream: different short messages
View(filter(chat,clusterCut==2)) # reactions on stream
View(filter(chat,clusterCut==3)) # one spammer
View(filter(chat,clusterCut==4)) # different spammers -- reddit memes and copypaste messages
View(filter(chat,clusterCut==5)) # warning bot
View(filter(chat,clusterCut==6)) # promotion bot
View(filter(chat,clusterCut==7)) # kappa spammer. Kappa is the most popular emoji on twitch.tv
```

# Task

It is not limited with users profiling. There are also game events. Let's try to analyze association of chat communication and game events in one game.
...
The number in each column of game event means its number in the data set. It is just an identifier and does not have much sense in rhe framework of one game.
